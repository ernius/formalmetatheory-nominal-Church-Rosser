...
The $\beta$-rule needs an \alp-conversion in its premises because our substitution operation normalises terms,  as a consequence of being defined with our alpha iteration principle. 
Next we prove the parallel reduction relation is equivariant. This is a basic property in nominal settings, establishing every ``well defined'' concept should not depend in any particular choice of names (names should be interchangable).

\begin{lemma}[Parallel Relation is equivariant]
\label{pequiv}
...
We have proved our relation is well beahaved with respect alpha conversion and the permutation operation, we now introduce next lemmas to prove that our parallel relation implies the next more classical one.
...
\AxiomC{$M \p M'$} 
\LeftLabel{\p \lam}
\UnaryInfC{$\lam x . M  \p  \lam x . M'$} \DisplayProof
\end{minipage}

\begin{minipage}{0.9\linewidth}
\center
\AxiomC{$\lam x . M \p \lam x . M'$}
\AxiomC{$N \p  N'$}
\LeftLabel{$\p \beta$}
...
% acaaaaaaaaaaaaaaaaaaaaaaaaaaaaa empece a editarrrrrrrrrrrr -------------------------------------------------------------
\begin{lemma}[\p\   \lam-elimination]
\label{lamelim}
\[ \lam x M \p M' \Rightarrow \\ 
\begin{array}{l}
\exists M'',  M \p M'', \\ M' \alpsym \lam x M'' , \lam x M \p \lam x M'' \\ 
\end{array} 
\]  
\end{lemma}
...
Finally, we can prove the diamond property of parallel reduction by a simple induction on term.
 ...
 %  \end{center}
  ...
  \begin{proof} By induction on term $M$. We show next the more interesting case, the rest are direct or similar to the detailed case.
    ...
    \begin{itemize}
    \item app. case: We do case analysis in both parallel relations, the interesting subcase is the combination of ($\p a$)\ and ($\p \beta$) rules, so our hypothesis are:
